...
class HtmxHeaders:
    ...
    def __bool__(self):
        return any(hasattr(self, o) for o in htmx_hdrs)
...
def _check_anno(arg, anno):
    ...
    if (
        anno is empty
        and arg.lower() not in _special_names
        and not any(s.startswith(arg.lower()) for s in ("request", "session"))
    ):
        return f"`{arg}` has no type annotation and is not a recognised special name, so is ignored."
    if (
        isinstance(anno, type)
        and not get_origin(anno)
        and issubclass(anno, (list, tuple))
        and not _is_body(anno)
    ):
        return f"`{arg}` uses bare `{anno.__name__}` annotation, so is ignored (use e.g. `{anno.__name__}[str]` instead)."
...
def _fix_anno(t, o):
    ...
    if origin is Union or origin is UnionType:
        origin = get_origin(t := first(o for o in get_args(t) if o != type(None)))  # type: ignore[arg-type]
    if origin in (list, List):
        t = first(o for o in get_args(t) if o != type(None))  # type: ignore[arg-type]
    d = {bool: str2bool, int: str2int, date: str2date, UploadFile: noop}
    res = d.get(t, t)
    assert res is not None, f"_fix_anno: no handler for type {t}"
    if origin in (list, List):
        return _mk_list(res, o)  # type: ignore[arg-type]
    if isinstance(t, type) and issubclass(t, (list, tuple)):
        return None
...
def _is_body(anno):
    return (
        issubclass(anno, (dict, ns))
        or hasattr(anno, "__from_request__")
        or _annotations(anno)
    )
...
async def parse_form(req: Request) -> FormData:
    ...
    if ctype.startswith("multipart/form-data"):
        ...
        if int(req.headers.get("Content-Length", "0")) <= len(boundary) + 6:
            return FormData()
...
class ApiReturn:
    ...
    async def __from_request__(cls, data, req):
        return cls(req.headers.get("accept") == "application/json")

    def __init__(self, isapi=False):
        self.isapi = isapi

    def __call__(self, norm=None, **kw):
        return kw if self.isapi else norm

    def __bool__(self):
        return bool(self.isapi)
...
class JSONResponse(JSONResponseOrig):
    ...
    def render(self, content: Any) -> bytes:
        def _default(o):
            return list(o) if is_listy(o) else str(o)
...
async def _find_p(conn, data, hdrs, arg: str, p: Parameter):
    ...
    if isinstance(anno, type) and not isinstance(anno, GenericAlias):
        if issubclass(anno, HtmxHeaders):
            return _get_htmx(hdrs)
        if issubclass(anno, Starlette):
            return conn.scope["app"]
        if issubclass(anno, HTTPConnection):
            return conn
        if issubclass(anno, State):
            return conn.scope["app"].state
        if anno is dict:
            return data
        if _is_body(anno):
            if "session".startswith(arg.lower()):
                return conn.scope.get("session", {})
            return await _from_body(conn, p, data)
    if msg := _check_anno(arg, anno):
        return warn(msg)
    if anno is empty:
        if arg.lower() == "ws" or "request".startswith(arg.lower()):
            return conn
        if "session".startswith(arg.lower()):
            return conn.scope.get("session", {})
        if arg.lower() == "scope":
        ...
        if arg.lower() == "state":
            return conn.scope["app"].state
        if arg.lower() == "auth":
            return conn.scope.get("auth", None)
        if arg.lower() == "send":
            assert not isinstance(conn, Request), (
                "`send` requires a websocket, not a `Request`"
            )
            raise NotImplementedError(
                "WebSocket send is not available without the FastHTML server"
            )
        if arg.lower() == "api":
            return ApiReturn(hdrs.get("accept") == "application/json")
        if arg.lower() == "body":
            return (await conn.body()).decode()
        if arg.lower() in ("hdrs", "ftrs", "bodykw", "htmlkw"):
            return getattr(conn, arg.lower())
        return None
    res = conn.path_params.get(arg, None)
    if res in (empty, None):
        res = conn.cookies.get(arg, None)
    if res in (empty, None):
        res = hdrs.get(snake2hyphens(arg), None)
    if res in (empty, None):
        res = conn.query_params.getlist(arg)
    if res == []:
        res = None
    if res in (empty, None):
    ...
    try:
        ...
        if isinstance(conn, Request):
            raise HTTPException(404, f"{conn.url.path}: {e}") from None
        raise


async def _find_ps(conn, data, hdrs, params):
    if conn.query_params:
        data |= dict(conn.query_params)
...
async def _handle(f, *args, **kwargs):
    return (
        ...
        else await run_in_threadpool(f, *args, **kwargs)
    )
...
def is_full_page(req, resp):
    ...
    if resp and any(getattr(o, "tag", "") == "html" for o in resp):
        return True
    return (
        "hx-request" in req.headers and "hx-history-restore-request" not in req.headers
    )
...
def _xt_cts(req, resp):
    ...
    if not is_full_page(req, resp):
        title = (
            []
            if any(getattr(o, "tag", "") == "title" for o in heads)
            else [Title(req.app.title)]
        )
...
def _resp(req, resp, cls=empty, status_code=200):
    ...
    if hasattr(resp, "__response__"):
        resp = resp.__response__(req)  # type: ignore[union-attr]
    if not (isinstance(cls, type) and issubclass(cls, Response)):
        cls = empty
    if isinstance(resp, FileResponse) and not os.path.isfile(
        cast(str | os.PathLike[str], resp.path)
    ):
        raise HTTPException(404, str(resp.path))
...
class Redirect:
    ...
    def __response__(self, req):
        if "hx-request" in req.headers:
            return HtmxResponseHeaders(redirect=self.loc)
...
fhjsscr = Script(
    src="https://cdn.jsdelivr.net/gh/answerdotai/fasthtml-js@1.0.12/fasthtml.js"
)
...
def qp(p: str, **kw) -> str:
    ...
    return p + (
        "?"
        + urlencode(
            {k: "" if v in (False, None) else v for k, v in kw.items()}, doseq=True
        )
        if kw
        else ""
    )
