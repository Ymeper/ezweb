...
__all__ = [
    ...
    "Div", "Dl", "Dt", "Em", "Embed", "Fencedframe", "Fieldset", "Figcaption", "Figure",
...
def ft_hx(tag, *c, target_id=None, hx_vals=None, hx_target=None, **kwargs):
    if hx_vals:
        import json
        kwargs["hx_vals"] = json.dumps(hx_vals) if isinstance(hx_vals, dict) else hx_vals
    if hx_target:
        kwargs["hx_target"] = "#" + hx_target.id if isinstance(hx_target, FT) else hx_target
    if target_id:
