...
__all__ = ['sid_scr', 'A', 'AX', 'Form', 'Hidden', 'CheckboxX', 'Script', 'Style', 'double_braces', 'undouble_braces',
           ...
           'run_js', 'HtmxOn', 'jsd', 'Fragment', 'Titled', 'Favicon', 'clear',
           'LdJson', 'LdContactPoint', 'LdOrg', 'LdWebsite', 'LdCourseInstance', 'LdCourse']
...
from .components import (
    ft_hx, ft_html, Div, H1, Input, Label, Link, Main, Title,
)

try: from IPython import display  # type: ignore[reportMissingModuleSource]
...
@delegates(ft_hx, keep=True)
def CheckboxX(checked:bool=False, label=None, value="1", id=None, name=None, **kwargs):
    "A Checkbox optionally inside a Label, preceded by a `Hidden` with matching name"
    if id and not name: name=id
    if not checked: checked=None
    res = Input(type="checkbox", id=id, name=name, checked=checked, value=value, **kwargs)  # type: ignore[arg-type]
...
def replace_css_vars(css, pre='tpl', **kwargs):
    ...
    return _re.sub(fr'var\(--{pre}-([\w-]+)\)', replace_var, css)
...
def run_js(js, id=None, **kw):
    ...
    if not id: id = _sys._getframe(1).f_code.co_name
    kw = {k:_json.dumps(v) for k,v in kw.items()}
...
def jsd(org, repo, root, path, prov='gh', typ='script', ver=None, esm=False, **kwargs)->Union[FT, str]:
...
def LdJson(typ, data:dict, script=False, extra=None, **kwargs)->Union[FT, dict]:
    ...
    return Script(_json.dumps(cts, indent=1), type="application/ld+json", **kwargs)

def LdContactPoint(contact_type:str, email:str=None, phone:str=None, script=False, **extra):
...
def LdOrg(name:str, url:str=None, logo:str=None, alt_name:str=None,
          same_as:list=None, contact_points:list=None, script=False, **extra):
...
def LdWebsite(name:str, url:str, script=False, **extra):
...
def LdCourseInstance(course_mode:str="Online", start_date:str=None, end_date:str=None,
                     location:dict=None, instructor:dict=None, script=False, **extra):
...
def LdCourse(name:str, description:str, provider:dict, course_instance:dict=None, script=False, **extra):
